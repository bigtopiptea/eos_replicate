<template>
    <Listbox as="div" v-model="selected">
      <div class="relative">
      <ListboxLabel class="absolute -top-[8px] left-[10px] block text-[11px] uppercase bg-white z-10">{{ label }}</ListboxLabel>
        ...
        <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
          <ListboxOptions class="absolute z-10 max-h-[200px] w-full overflow-auto bg-white p-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none text-[11px]">
            <ListboxOption>
            ...
            <ListboxOption as="template" v-for="option in options" :key="option" :value="option" v-slot="{ active, selected }">
              <li :class="[active ? 'bg-gray-200 text-black' : 'text-gray-900', 'relative cursor-default select-none py-1 pl-3 pr-9']" class="flex gap-[10px] uppercase">
                <input type="checkbox" name="" id="" :checked="selected ? true : false">
