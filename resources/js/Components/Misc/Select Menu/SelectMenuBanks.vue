<template>
    <Combobox as="div" @input="$emit('update:modelValue', $event.target.value)" :value="props.modelValue.name"
        ...
        <div class="relative">
            <ComboboxLabel
                class="text-[10px] font-bold absolute bottom-7 text-blue-500 bg-white ml-3 uppercase tracking-widest">
                {{ label }}
            ...
            <ComboboxOptions v-if="filteredData"
                class="absolute bg-white border border-gray-300 z-10 mt-1 w-full overflow-auto">
                <ComboboxOption class="" v-for="name in filteredData" :key="name.id" :value="name" as="template"
                    v-slot="{ active, selected }">
                    <li
                        :class="['relative cursor-pointer uppercase select-none py-2 px-3', active ? 'bg-indigo-600  text-white ' : 'text-gray-900']">
                        <span :class="['block truncate', selected && 'font-semibold']">
                            {{ name.name }}
                        </span>

                        <span v-if="selected"
                            :class="['absolute inset-y-0 right-0 flex items-center pr-4', active ? 'text-white  font-bold font-rubik' : 'text-indigo-600']">
