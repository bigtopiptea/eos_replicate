            ...
            navItems: [
                ...
                },
                {
                    ...
                    children: [
                        ...
                        {
                            to: "/app/accounting/transactions/view-je",
                            label: "Acc. View JE",
                            children: [],
                            icon: CircleCheckIcon,
                        },
...
<template>
    <!-- Sidebar -->
    <div
        class="border-gradient-top border-t-[3px] z-10 fixed inset-x-0 w-full"
    ></div>
    <div class="flex bg-gray-100">
        <div
            class="flex flex-col inset-y-0 fixed z-10 flex-shrink-0 border-r-[2px] my-[0.15rem] nav w-64 bg-white"
            :class="!isToggled ? 'w-32' : '64'"
        >
            <ChevronLeftIcon
                @click="isToggled = !isToggled"
                class="h-6 absolute cursor-pointer -right-[0.80rem] top-[140px] transition-all"
                :class="isToggled ? 'rotate-0' : 'rotate-180'"
            />
            <div class="flex items-center justify-center z-10 py-6 text-white">
                <img
                    src="../../../assets/images/EosMaker.png"
                    class="h-12 m-auto duration-300"
                    alt=""
                />
            </div>
            <div>
                <img
                    class="m-auto h-[100px] w-[100px] my-5 rounded-full"
                    :src="user.avatar"
                    alt=""
                />
                <div v-if="isToggled" class="text-center">
                    <div
                        class="text-[11px] font-rubik tracking-widest uppercase mt-2 text-[#3E3E3E]"
                    >
                        {{
                            user.first_name +
                            " " +
                    ...
                    <div
                        class="text-[11px] font-rubik tracking-widest font-bold uppercase text-[#3E3E3E]"
                    >
                        {{ user.role }}
                    </div>
                </div>
            ...
            <nav
                v-if="isToggled"
                class="flex-grow border-r border-gray-200 py-2 overflow-y-scroll"
            >
                <ul class="space-y-1">
                    <li
                    ...
                    >
                        <router-link
                            ...
                            "
                            class="uppercase text-gray-900 group flex items-center justify-between pl-5 py-2 text-[11px] tracking-widest font-medium hover:bg-[#3e3e3e] hover:text-white duration-300"
                            :to="menuItem.to"
                        >
                            <div class="flex">
                                <component
                            ...
                            <component
                                class="h-4 w-6 duration-300 mr-4 -rotate-180"
                                :is="menuItem.toggleOffIcon"
                            >
                            </component>
                        </router-link>
                        <ul v-if="menuItem.children && menuItem === activeItem">
                            ...
                            >
                                ...
                                <router-link
                                    ...
                                    class="px-[25px] text-[11px] whitespace-nowrap gap-1 w-full flex items-center duration-300 my-1 hover:text-[#f1721a] py-2 uppercase tracking-wider"
                                    :to="subItem.to"
                                >
                                    <SelectedRadioButton
            ...
            <div
                v-else
                class="flex-grow border-r border-gray-200 overflow-y-scroll h-[300px]"
            >
                <div id="dropdown" class="ml-[8.5rem]">
                    <ul
                        class="bg-white shadow-xl border border-gray-200 max-h-[300px] overflow-y-auto absolute text-gray-700"
                        ...
                        <li v-for="links in navItems" :key="links">
                            ...
                            <div v-if="links.children && links === activeItem">
                                <router-link
                                    ...
                                    class="px-6 py-[0.5rem] tracking-widest text-[10px] border-b whitespace-nowrap w-full gap-2 flex items-center duration-300 hover:text-white hover:bg-[#F9951E] uppercase"
                                >
                ...
                <ul class="space-y-1">
                    ...
                    >
                        <router-link
                            ...
                            "
                            :to="menuItem.to"
                            class="uppercase text-[8px] hover:text-[#FA961E] duration-300"
                        >
                            <component
        ...
        <div
            class="flex flex-col flex-grow ml-64"
            :class="!isToggled ? 'ml-32' : ''"
        >
            <!-- Navbar -->
            <div
                class="fixed bg-white mt-[0.15rem] right-0 left-64 z-10 px-4 py-2"
                :class="!isToggled ? 'left-32' : ''"
            >
                <div class="min-w-0 flex-1 flex justify-between">
                    <div v-if="!isToggled">
                        <h1 class="uppercase text-[11px]">
                            {{ user.first_name }} {{ user.last_name }}
                        </h1>
                        <h1 class="uppercase text-[11px] font-bold">
                            {{ user.role }}
                        </h1>
                    </div>
                    <div></div>
            ...
            <div
                v-if="isLogout"
                class="bg-white rounded-sm border border-gray-300 w-[15%] fixed right-4 z-10 top-12"
            >
                <div class="text-center py-4">
                    <div
                        class="px-4 m-auto w-full flex items-center justify-center"
                    >
                        <img
                ...
                <ul class="p-2">
                    <button
                        @click="logout()"
                        class="uppercase px-[1rem] font-[700] w-full hover:bg-gray-300 py-2 text-[#3e3e3e] text-left text-[12px] rounded-sm tracking-widest"
                        to=""
                    >
                        Logout
                    </button>
            ...
            <div
                class="flex-grow bg-[#ECECEC] min-h-screen px-4 py-6 my-10 mt-6"
            >
            ...
            <div
                class="border-b-[2px] border-gradient-bottom inset-x-0 border-gray-100 fixed bottom-0 shadow-sm bg-white w-full px-4 py-3 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8"
            >
                <div class="min-w-0 flex-1">
                    <h1
                        class="text-right uppercase text-[12px] tracking-widest"
                    >
                        Date today:
                        {{ currentDate() }}
                    </h1>
        ...
        <div class="flex flex-col justify-between h-full pb-3">
            <div class="input-area mx-10">
                <div class="profile-upload flex justify-evenly items-center">
                    ...
                    />
                    <BorderButton
                        @click="profileSlideoverOpen = !profileSlideoverOpen"
                        @click.prevent="slideOverToggle()"
                        :buttonLabel="'UPLOAD'"
                        :buttonSize="'h-auto w-[100px]'"
            ...
            <div class="flex justify-center">
                <BorderButton
                    ...
                    :buttonLabel="'CLOSE'"
                    :buttonPadding="'p-2'"
                    :buttonTextColor="'text-[#3e3e3e]'"
                    :buttonBorderColor="'border-[#3e3e3e]'"
                    :buttonHover="'hover:bg-[#3E3E3E]'"
                    :buttonTextSize="'text-[15px]'"
                />
            </div>
        </div>
    </Slideover>
    ...
    <Slideover
        :show="profileSlideoverOpen"
        @close="profileSlideOverToggle"
        :title="'CHANGE PHOTO'"
        :iconShow="paraIcon"
    >
        <div class="flex flex-col justify-between items-center h-full pb-5">
            <div class="py-5 mx-5 w-4/5">
                <div
                    class="flex flex-col items-center border-dashed border-2 border-#7F7F7F rounded-md p-5"
                >
                    <div class="text-center mb-5">
                        ...
                        >
                            <BorderButton
                                :buttonLabel="'browse'"
                                :buttonSize="'h-auto w-[100px]'"
                                :buttonTextColor="'text-[#EE3E2C]'"
                                :buttonBorderColor="'border-[#EE3E2C]'"
                                :buttonHover="'hover:bg-[#EE3E2c]'"
                                :buttonPadding="'px-4 py-1'"
                                :buttonTextSize="'text-[12px]'"
                            />
                            <input
                                class="absolute w-[100px] opacity-0"
                                type="file"
                                accept=".jpg, .jpeg, .png"
                            />
                        </div>
                    ...
                    <div class="text-center text-[10px]">
                        <p>
                            MAX FILE SIZE: <span class="font-bold">1MB</span
                            ><br />
            ...
            <div class="flex justify-between w-4/5">
                <BorderButton
                    @click.prevent="profileSlideOverToggle()"
                    :buttonLabel="'CANCEL'"
                    :buttonPadding="'p-2'"
                    :buttonTextColor="'text-[#3e3e3e]'"
                    :buttonBorderColor="'border-[#3e3e3e]'"
                ...
                <BorderButton
                    @click="
                        adjustProfileSlideoverOpen = !adjustProfileSlideoverOpen
                    "
                    @click.prevent="profileSlideOverToggle()"
                    :buttonLabel="'SAVE'"
                    :buttonPadding="'p-2'"
    ...
    <Slideover
        :show="adjustProfileSlideoverOpen"
        @close="adjustProfileSlideoverToggle"
        :title="'CHANGE PHOTO'"
        :iconShow="paraIcon"
    >
        <div class="flex flex-col justify-between items-center py-5 h-full">
            <div class="flex flex-col items-center">
                <div
                    class="flex justify-center items-center w-auto h-auto p-7 bg-#F9951E rounded-md relative"
                >
                    <div
                    ...
                    <div
                        class="flex justify-center items-center w-72 h-52 bg-#EAEAEA"
                    >
                        <img
                            src="https://pbs.twimg.com/profile_images/1564398871996174336/M-hffw5a_400x400.jpg"
                            alt="user-logo"
                            class="w-48 h-48 rounded-full border-2 border-[#c6c3bd]"
                        />
                    </div>
                </div>
                <div class="mt-2">
            ...
            <div class="flex justify-between w-4/5">
                <BorderButton
                    @click.prevent="adjustProfileSlideoverToggle()"
                    :buttonLabel="'CANCEL'"
                    :buttonPadding="'p-2'"
                    :buttonTextColor="'text-[#3e3e3e]'"
                    :buttonBorderColor="'border-[#3e3e3e]'"
