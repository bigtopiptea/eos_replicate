    ...
    methods: {
        ...
        async updateMultipleServiceFee() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.put("/api/fees/update/amendment-fees", {
                        ids: this.selectedIds,
                        amendment_fee: this.amendment_fee,
                    }).then((response) => {
        ...
        openServiceFee() {
            if (this.selectedAction.label === 'Amendment Fee') {
                this.isOpen = true;
            }
        },
    },
            ...
            <div class="w-full flex items-center space-x-4">
                ...
                <form @submit.prevent="updateMultipleServiceFee()">
                    <div v-if="isOpen" class="-mt-6 flex items-center space-x-2">
                        <div>
                            <InputLabel label="Amendment Fee" />
                            <SettingsInput :class="{ 'border border-mid-red': error.amendment_fee || error[0] }"
                                v-model="amendment_fee" placeholder="Enter Amendment Fee" class="w-full uppercase" />
                            <span class="text-mid-red uppercase text-[10px] absolute" v-if="error.amendment_fee">
                                {{ error.amendment_fee[0] }}
                            </span>
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    <tr v-for="item in fees_data.data" :key="item.id" class="divide-x divide-gray-200">
                                        ...
                                        <td
                                            ...
                                            <span v-if="selected != item.id">{{ item.amendment_fee }}</span>
                                            <form @submit.prevent="updateAmount(item.id, item.amendment_fee)" v-else>
                                                <div class="relative -left-[6.8rem] -top-3">
                                                    <input type="double" v-model="item.amendment_fee"
                                                        class="absolute text-center border-2 border-[#F9951E] w-full font-rubik-light py-1" />
                                                </div>
                                            </form>
                                        </td>
                                        <td
                                            class="uppercase p-2  text-center text-[12px] font-rubik-light tracking-wider">
                                            {{
                                                item.updated_at ?
                                        ...
                                        <td
                                            class="text-center uppercase p-2  text-[12px] font-rubik-light tracking-wider">
                                            <button v-if="selected != item.id" @click.prevent="editAmount(item.id)">
                                                <EditIcon class="text-[#f0921e] h-5" />
                                            </button>
                                            <button @click.prevent="updateAmount(item.id, item.amendment_fee)"
