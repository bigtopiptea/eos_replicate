...
export default {
    ...
    methods: {
        async getFees(option) {
            this.isLoading = true;
            await axios
                .get(`/api/fees?page=${this.fees_data.current_page}`, {
                    params: {
        ...
        async updateAmount(id, amount, branch_id) {
            Swal.fire({
        ...
        async getBranchCurrency() {
            await axios.get("/api/fees/branch-currency").then((response) => {
        ...
        async updateMultipleServiceFee() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios
                        .put("/api/fees/update/service-fees", {
                        ...
                        .then((response) => {
                            if (this.selectedIds.length === 0) {
                                ...
                                this.error = [
                                    (message =
                                        "Select item to update service fee"),
                                ];
                            } else {
                                successMessage(
                                    "Success!",
        ...
        allSelected: {
            get: function () {
                if (this.fees_data.data) {
                    return (
                        this.fees_data.data.length === this.selectedIds.length
                    );
                }
            },
            set(value) {
                if (value) {
                    this.selectedIds = this.fees_data.data.map(
                        (item) => item.id
                    );
                } else {
                    this.selectedIds = [];
        ...
        <div class="flex items-center justify-between px-8 mt-4">
            <div class="w-full">
                ...
                <!-- <v-select @option:selected="getFees" v-model="queryField" label="label" :options="branch_currency"
                        placeholder="Sort " class="w-[30%] border border-gray-300 shadow-sm"></v-select> -->
            </div>
            <div class="w-full">
                ...
                <form @submit.prevent="updateMultipleServiceFee()">
                    <div v-if="isOpen" class="-mt-6 flex items-center space-x-2">
                        <div>
                            ...
                            <SettingsInput :class="{
                                'border border-mid-red':
                                    error.service_fee || error[0],
                            }" v-model="service_fee" placeholder="Enter Service Fee" class="w-full uppercase" />
                            <span class="text-mid-red uppercase text-[10px] absolute" v-if="error.service_fee">
                                {{ error.service_fee[0] }}
                            </span>
                            ...
                            <table class="min-w-full divide-y divide-gray-300">
                                <thead class="bg-[#D7D7D7]">
                                    <tr class="divide-x divide-gray-200">
                                        ...
                                        <th scope="col"
                                            class="p-2 whitespace-nowrap uppercase tracking-wider font-rubik-light text-center text-[12px] font-semibold text-gray-900">
                                            Type of Fee
                                        </th>
                                        <th scope="col"
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    <tr v-for="item in fees_data.data" :key="item.id" class="divide-x divide-gray-200">
                                        <td
                                            class="space-x-3 font-thin whitespace-nowrap uppercase p-2 text-[12px] font-rubik-light tracking-wider">
                                            <input v-model="selectedIds" :value="item.id" type="checkbox" />
                                            <span>
                                                {{
                                                    item.biller.transaction_type
                                                        .meta_value
                                                }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="uppercase p-2 text-[12px] text-center font-rubik-light tracking-wider">
                                            {{
                                                item.amount_range
                                                ? item.amount_range.replace(/"/g, "")
                                                : "-"
                                            }}
                                        </td>
                                        <td
                                        ...
                                        <td
                                            class="whitespace-nowrap text-center w-full uppercase p-2 text-[12px] font-rubik-light tracking-wider">
                                            <span v-if="selected !== item.id">{{
                                                item.type_of_fee
                                            }}</span>
                                            <v-select v-model="type_of_fee" v-else placeholder="Type of Fee"
                                                class="border border-gray-300 w-full" label="label" :options="typeOfFee">
                                            </v-select>
                                        </td>
                                        <td
                                            ...
                                            <span v-if="selected !== item.id">{{
                                                ...
                                                "Percentage"
                                                ? item.service_fee + "%"
                                                : item.service_fee.toFixed(
                                                    2
                                                )
                                            }}</span>
                                            <!--                                            <form @submit.prevent="updateAmount(item.id, item.service_fee)" v-else>-->
                                            <div v-else class="relative -left-[5.18rem] -top-3">
                                                <input type="double" v-model="item.service_fee"
                                                    class="absolute duration-300 text-center border-2 border-[#F9951E] w-full font-rubik-light py-1" />
                                            </div>
                                            <!--                                            </form>-->
                                        </td>
                                        <td
                                            class="whitespace-nowrap uppercase p-2 text-left text-[12px] font-rubik-light tracking-wider">
                                            {{
                                                item.updated_at
                                                ? $moment(
                                                    item.updated_at
