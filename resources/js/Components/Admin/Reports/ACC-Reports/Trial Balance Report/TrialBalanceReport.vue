...
import TrialBalanceTable from "./Tables/TrialBalanceTable.vue";
export default {
    name: 'Accounts Payable Report',
    components: {
        NormalButton, SearchIcon, ListIcon, DateInput,
        CheckboxSelectMenuThree, TrialBalanceTable,
    },
    data() {
        return {
            accountOptions:[
...
<template>
    <div class="w-full h-screen bg-white">
        <div class="flex flex-col gap-[15px] min-w-full px-3 pt-10 pb-5">
            <div class="flex gap-[10px] w-[85%] mx-[12px]">
                <div class="flex items-center gap-[10px] w-[50%]">
                    <div class="w-[65%]">
                        <CheckboxSelectMenuThree v-model="selectedAccount" :label="'ACCOUNT'" :inputWidth="'w-12/12'" :options="accountOptions" :placeholder="'SELECT ACCOUNT NAME'" :withCheckbox="true" @input="updateSelectedAccount"/>
                    </div>
                </div>
            </div>
        ...
        {{  }}
        <!-- MAIN CONTENT -->
        <div v-if="this.selectedAccount && this.startDate && this.endDate">
            <TrialBalanceTable :Report="this.selectedAccount" :StartDate="this.startDate" :EndDate="this.endDate"/>
        </div>
