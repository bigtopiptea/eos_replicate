...
export default {
    ...
    components: {
        ...
        CheckboxSelectMenu, PaymentToSuppliersTable
    },
    data() {
        return {
            supplier:[
    ...
    <div class="w-full h-screen bg-white">
        <div class="flex flex-col gap-[15px] min-w-full px-3 pt-10 pb-5">
            <div class="flex gap-[10px] w-[85%] mx-[12px]">
                <div class="w-[40%]">
                    <CheckboxSelectMenu v-model="selectedSupplier" :label="'supplier'" :inputWidth="'w-12/12'"  :placeholder="'SELECT SUPPLIER'" :options="supplier"/>
                </div>
            </div>
        ...
        <!-- MAIN CONTENT -->
        <div v-if="this.selectedSupplier">
            <PaymentToSuppliersTable :Supplier="selectedSupplier.name"/>
        </div>
        <div v-else>
